...
void FileInfo::slotGet()
{
    ...
    qint64    size         = info.size();         // 获得info对象的大小
    QDateTime created      = info.birthTime();    // 获得QFileInfo对象创建的时间
    QDateTime lastModified = info.lastModified(); // 获得QFileInfo对象最后修改的时间
    QDateTime lastRead     = info.lastRead();     // 获得QFileInfo对象最后访问的时间
    ...
    bool isWritable   = info.isWritable();   // 判断QFileInfo对象的读属性
    bool isExecutable = info.isExecutable(); // 判断QFileInfo对象的可执行属性
