    ...
    : QDialog( parent )
{
    ...
    connect( ui.comboDriver, &QComboBox::currentTextChanged, this, &ConnDlg::driverChanged );
    ui.status_label->setText( tr( "准备连接数据库!" ) );
}

ConnDlg::~ConnDlg() {}

QString ConnDlg::driverName() const { return ui.comboDriver->currentText(); }

QString ConnDlg::databaseName() const { return ui.editDatabase->text(); }

QString ConnDlg::userName() const { return ui.editUsername->text(); }

QString ConnDlg::password() const { return ui.editPassword->text(); }

QString ConnDlg::hostName() const { return ui.editHostname->text(); }

int ConnDlg::port() const { return ui.portSpinBox->value(); }
...
void ConnDlg::createDb()
{
    QSqlQuery query;
    query.exec(
        "create table factory (id int primary key,manufactory varchar(40), address varchar(40))" );
    ...
    query.exec( "create table cars (carid int primary key, name varchar(50), factoryid int, year "
                "int, foreign key(factoryid) references factory(id))" ); //(c)
