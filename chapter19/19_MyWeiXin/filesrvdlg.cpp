...
FileSrvDlg::~FileSrvDlg() { delete ui; }

void FileSrvDlg::cntRefused()
{
    myTcpSrv->close();
    QMessageBox::warning( nullptr, QObject::tr( "提示" ), "对方拒绝接收" );
}

void FileSrvDlg::closeEvent( QCloseEvent * ) { on_srvClosePushButton_clicked(); }
...
void FileSrvDlg::refreshProgress( qint64 bynum )
{
    ...
    ui->sfileSizeLineEdit->setText( tr( "%1" ).arg( myTotalBytes / ( 1024 * 1024 ) ) +
                                    "MB" ); // 填写文件总大小栏
    ui->sendSizeLineEdit->setText( tr( "%1" ).arg( mySendBytes / ( 1024 * 1024 ) ) +
                                   "MB" ); // 填写已发送栏
...
void FileSrvDlg::on_sendFilePushButton_clicked()
{
    ...
    {
        QMessageBox::warning( nullptr, QObject::tr( "异常" ), "打开TCP端口出错，请检查网络连接" );
        close();
        return;
    }

    emit sendFileName( myFileName ); // 发送信号
}
