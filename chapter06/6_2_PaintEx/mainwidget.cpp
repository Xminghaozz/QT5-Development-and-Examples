    ...
    : QWidget( parent )
{
    ...
    shapeLabel    = new QLabel( tr( " 形状：" ) ); // 形状选择下拉框列表
    ...
    connect( shapeComboBox, &QComboBox::activated, this, &MainWidget::ShowShape );
    ...
    connect( penWidthSpinBox, &QSpinBox::valueChanged, this, &MainWidget::ShowPenWidth );
    ...
    connect( penStyleComboBox, &QComboBox::activated, this, &MainWidget::ShowPenStyle );
    ...
    connect( penCapComboBox, &QComboBox::activated, this, &MainWidget::ShowPenCap );
    ...
    connect( penJoinComboBox, &QComboBox::activated, this, &MainWidget::ShowPenJoin );

    fillRuleLabel    = new QLabel( tr( "填充模式：" ) ); // 填充模式选择下拉列表框
    fillRuleComboBox = new QComboBox;
    fillRuleComboBox->addItem( tr( "Odd Even" ), Qt::OddEvenFill );
    fillRuleComboBox->addItem( tr( "Winding" ), Qt::WindingFill );
    connect( fillRuleComboBox, &QComboBox::activated, this, &MainWidget::ShowFillRule );
    ...
    connect( spreadComboBox, &QComboBox::activated, this, &MainWidget::ShowSpreadStyle );
    ...
    connect( brushStyleComboBox, &QComboBox::activated, this, &MainWidget::ShowBrush );
    ...
    ShowShape( shapeComboBox->currentIndex() ); // 显示默认的布局
}
...
void MainWidget::ShowPenStyle( int styleValue )
{
    ...
    int          value = penWidthSpinBox->value();
    Qt::PenStyle style =
        Qt::PenStyle( penStyleComboBox->itemData( styleValue, Qt::UserRole ).toInt() );
    Qt::PenCapStyle cap = Qt::PenCapStyle(
        penCapComboBox->itemData( penCapComboBox->currentIndex(), Qt::UserRole ).toInt() );
    Qt::PenJoinStyle join = Qt::PenJoinStyle(
        penJoinComboBox->itemData( penJoinComboBox->currentIndex(), Qt::UserRole ).toInt() );
...
void MainWidget::ShowPenCap( int capValue )
{
    ...
    Qt::PenStyle style = Qt::PenStyle(
        penStyleComboBox->itemData( penStyleComboBox->currentIndex(), Qt::UserRole ).toInt() );
    Qt::PenCapStyle cap =
        Qt::PenCapStyle( penCapComboBox->itemData( capValue, Qt::UserRole ).toInt() );
    Qt::PenJoinStyle join = Qt::PenJoinStyle(
        penJoinComboBox->itemData( penJoinComboBox->currentIndex(), Qt::UserRole ).toInt() );
    paintArea->setPen( QPen( color, value, style, cap, join ) );
}
...
void MainWidget::ShowPenJoin( int joinValue )
{
    ...
    Qt::PenJoinStyle join =
        Qt::PenJoinStyle( penJoinComboBox->itemData( joinValue, Qt::UserRole ).toInt() );
    paintArea->setPen( QPen( color, value, style, cap, join ) );
}

void MainWidget::ShowSpreadStyle()
{
    spread = QGradient::Spread(
        spreadComboBox->itemData( spreadComboBox->currentIndex(), Qt::UserRole ).toInt() );
}

void MainWidget::ShowFillRule()
{
    Qt::FillRule rule = Qt::FillRule(
        fillRuleComboBox->itemData( fillRuleComboBox->currentIndex(), Qt::UserRole ).toInt() );
...
void MainWidget::ShowBrush( int value )
{
    ...
    Qt::BrushStyle style =
        Qt::BrushStyle( brushStyleComboBox->itemData( value, Qt::UserRole ).toInt() );
    ...
    {
        paintArea->setBrush( QBrush( QPixmap( ":/Img/image.png" ) ) );
    }
    else
    {
