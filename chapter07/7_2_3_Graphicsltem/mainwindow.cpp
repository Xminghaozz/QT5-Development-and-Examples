...
void MainWindow::slotAddEllipseItem() // 在场景中添加一个椭圆形图元
{
    ...
    scene->addItem( item );
    item->setPos( ( rand() % int( scene->sceneRect().width() ) ) - 200,
                  ( rand() % int( scene->sceneRect().height() ) ) - 200 );
}
...
void MainWindow::slotAddPolygonItem() // 在场景中加入一个多边形图元
{
    ...
    QGraphicsPolygonItem *item = new QGraphicsPolygonItem( QPolygon( v ) );
    item->setBrush( QColor( rand() % 256, rand() % 256, rand() % 256 ) );
    item->setFlag( QGraphicsItem::ItemIsMovable );
    scene->addItem( item );
    item->setPos( ( rand() % int( scene->sceneRect().width() ) ) - 200,
...
{
    QGraphicsPixmapItem *item = scene->addPixmap( QPixmap( ":/Img/image.png" ) );
    item->setFlag( QGraphicsItem::ItemIsMovable );
    item->setPos( ( rand() % int( scene->sceneRect().width() ) ) - 200,
                  ( rand() % int( scene->sceneRect().height() ) ) - 200 );
}
...
void MainWindow::slotAddAnimationItem() // 在场景中加入一个动画沁星星
{
}
